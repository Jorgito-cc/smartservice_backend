...
module.exports = {
    ...
    async dashboard(req, res) {
        try {
            ...
            // Ingresos
            const ingresosTotales = await PagoServicio.sum("monto", {
                where: { estado: "pagado" }
            }) || 0;

            const ingresosMes = await PagoServicio.sum("monto", {
                where: {
                    estado: "pagado",
            ...
            const topTecnicos = await ServicioAsignado.findAll({
                ...
                group: ["id_tecnico", "Usuario.id_usuario", "Usuario.nombre", "Usuario.apellido", "Usuario.foto"],
    ...
    async tecnicosTop(req, res) {
        try {
            ...
            const result = await ServicioAsignado.findAll({
                ...
                group: ['ServicioAsignado.id_tecnico', 'Usuario.id_usuario', 'Usuario.nombre', 'Usuario.apellido', 'Usuario.foto', 'Tecnico.id_tecnico', 'Tecnico.calificacion_promedio'],
            ...
            console.error(error);
            res.status(500).json({ msg: "Error obteniendo técnicos top" });
        }
    },
    ...
    async resumenGeneral(req, res) {
        try {
            const [totalServicios, serviciosCompletados, totalIngresos, totalClientes, totalTecnicos] = await Promise.all([
                ...
                ServicioAsignado.count({ where: { estado: 'finalizado' } }),
